...
| **弹幕朗读** | Edge 在线（Azure 神经音，零配置）+ GPT-SoVITS 克隆 + 系统内置 三引擎；礼物连送聚合，不刷屏 |
...
**① Edge 在线（推荐 · 零配置）**：朗读 TAB 默认即此项 → 勾「启用」→「测试朗读」。调 Edge 浏览器的 Azure 神经音色（晓晓 / 云希等），**免部署、免 key、免参考音频**，音质远胜系统内置、与 GPT-SoVITS 中性朗读持平。需联网（直播本就联网）；非官方端点，偶发失效时切系统内置兜底。
